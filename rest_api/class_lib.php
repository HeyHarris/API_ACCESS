...
class lib {
    ...
    */
    ...
    Input: Menu name, Assoc array to build menu from, various optional features for menu
    Returns: String containing the menu
    */
    ////////////////////////////////////////////////////////////////////////////////////////////
    static function menu_from_assoc_array($name, &$array, $dummy_option='', $selected='', $multiple='', $event_handler='', $styles='') {
        ...
        $menu = "<select name=\"$name\" id=\"$id\" $multiple $event_handler $styles>\n";

        if ($dummy_option) {
            // Option such as "Choose Item" for first item of single selection menu
            $menu .= "<option value=\"\" ";
            if ( $selected === '' || (is_array($selected) && in_array('',$selected)) ) {
                $menu .= "selected='yes'";
            }
